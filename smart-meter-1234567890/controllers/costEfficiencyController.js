...
const getCostEfficiency = async (req, res) => {
  try {
    // 1. Get latest readings (use more if you want; 10 for demo)
    const readings = await Energy.find().sort({ timestamp: -1 }).limit(10);
    ...
    // 2. Compute cost for today ("period")
    const totalPower = readings.reduce((acc, r) => acc + (r.power || 0), 0);
    // Convert to kWh: 1 kWh = (1000 W) × (60m), readings assumed 1/min
    const totalEnergyKWh = (totalPower / 1000) * (1 / 60) * readings.length;
    const costData = [
      {
        period: new Date().toISOString().split("T")[0],
        amount: (totalEnergyKWh * 7.5).toFixed(2) // Assume ₹7.5 per kWh; adjust as needed
      }
    ];

    // 3. Calculate basic efficiency (actual vs. predicted)
    const totalActual = readings.reduce(
      (acc, r) => acc + (r.energy || r.energyConsumed || r.power || r.value || 0),
      0
    );

    // 4. Get predicted usage from AI (Optional, else fallback)
    let predictedUsage = null;
    try {
      const aiRes = await axios.get("http://localhost:5000/api/predict");
      predictedUsage = aiRes.data.predicted_usage;
    } catch (err) {
      predictedUsage = totalActual * 1.05; // fallback: 5% above current usage
    }
    const actualEfficiency =
      predictedUsage > 0
        ? ((predictedUsage - totalActual) / predictedUsage) * 100
    ...
    // 5. Get AI-powered recommendations — Uses Python service for dynamic tips!
    let aiRecommendations = [];
    try {
      // Instead of GET, POST readings to your own AI microservice (dynamic tips!)
      const recRes = await axios.post(
        "http://localhost:5000/api/recommendations",
        {
          powerConsumption: readings.map((r) => r.power),
      ...
      // Accept your AI service response format; adjust as needed:
      aiRecommendations =
        recRes.data.aiResult?.recommendations || recRes.data.recommendations || [];
    } catch (err) {
      // fallback static tips
      aiRecommendations = [
        { title: "Unplug idle devices", description: "Save up to 10% energy." },
        { title: "Use LED bulbs", description: "LEDs consume up to 80% less power than traditional bulbs." }
      ];
    }

    // 6. Send all dashboard info to the frontend
    res.json({
      success: true,
      costData,
      efficiencyData: {
